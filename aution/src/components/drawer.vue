<template>
  ...
  >
    ...
    <h1>Colors checked: {{ selectedColors.join(", ") }}</h1>
      ...
      >
        <div class="pt-6">
          <div class="space-y-6">
            ...
            >
              <input
                :id="'filter-mobile-rarity-' + index"
                :name="'rarity[]'"
                :value="color.value"
                type="checkbox"
                class="h-4 w-4 rounded border-gray-300 text-blue-700 focus:ring-indigo-500"
                :checked="isChecked(color.value)"
                @change="updateSelectedCategories(color.value)"
              />
              <label
                :for="'filter-mobile-rarity-' + index"
      ...
      >
        <h2 id="accordion-collapse1-heading-1">
          <button
            type="button"
            class="flex w-full bg-inherit items-center justify-between"
            data-accordion-target="#accordion-collapse1-body-1"
            aria-expanded="true"
        ...
        <div
          id="accordion-collapse1-body-1"
          class="hidden px-2"
          aria-labelledby="accordion-collapse1-heading-1"
        >
          <div class="pt-6">
            <div class="space-y-6">
              <div
              ...
              >
                <input
                  ...
                  type="checkbox"
                  class="h-4 w-4 rounded border-gray-300 text-blue-700 focus:ring-indigo-500"
                  :checked="isChecked(rarity.value)"
                  @change="updateSelectedCategories(rarity.value)"
                />
                <label
                  :for="'filter-mobile-rarity-' + index"
                  class="ml-3 min-w-0 flex-1 text-gray-500"
            ...
            <div class="space-y-6">
              ...
              >
                <input
                  ...
                  @change="updateSelectedCategories(size.value)"
                />
                <label
                  :for="'filter-mobile-rarity-' + index"
                  class="ml-3 min-w-0 flex-1 text-gray-500"
...
export default {
  ...
  methods: {
    updateSelectedCategories(categoryValue) {
      const index = this.selectedColors.indexOf(categoryValue);
      if (index === -1) {
        this.selectedColors.push(categoryValue);
      } else {
    ...
    isChecked(categoryValuec) {
      return this.selectedColors.includes(categoryValuec);
    },
    range() {
      this.mintrigger();
      this.maxtrigger();
